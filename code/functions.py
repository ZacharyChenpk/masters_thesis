...
def get_user_heur1(df):
    ...
    users = [user for user in users if len(user.adr)>1] #Remove all users that only have one address assigned to them
...
def get_user_features_df(df, users):
    ...
    user_input_df['total_sent'] = (df.groupby(['input_user', 'id_txi']).sum()['input_val']/df.groupby(['input_user', 'id_txi']).size()).groupby('input_user').sum()
    #user_input_df['total_sent'] = (df['input_val'] / df['num_txo']).groupby(df['input_user']).sum()

    #print(list(user_input_df.columns))
    ...
    user_out_df['total_rec'] = (df.groupby(['output_user', 'id_txo_out']).sum()['output_val']/df.groupby(['output_user', 'id_txo_out']).size()).groupby('output_user').sum()
    #user_out_df['total_rec'] = (df['output_val'] / df['num_txi']).groupby(df['output_user']).sum()

    #print(list(user_out_df.columns))

    # Merge input and output user features
    user_df = user_input_df.merge(user_out_df, how='outer', left_index=True, right_index=True, sort=False)
    user_df = user_df[user_df.index.to_series().apply(lambda x: isinstance(x,int))]
    ...
    _num_cadr = [len(_user.cadr) for _user in users]

    user_df['num_blocks_active'] = _blocks
    user_df['num_adr'] = _num_adr
    user_df['num_cadr'] = _num_cadr
    ...
    user_df['ratio'] = round(user_df['num_receiving_tx']/user_df['num_sending_tx'], 2)
...
def tag_users(users, user_df, df):
    ...
    dic_adr_to_cat = defaultdict(set)
    #dic_adr_to_site = defaultdict(set)
    starttime = time.time()
    ...
    for category in categories:
        ...
        #dummy_df = category_df[['address', 'website']].copy()
        #dummy_df = dummy_df.set_index('address')
        x = all_adr.intersection(set(category_df.address))
        if x:
            for adr in x:
                dic_adr_to_cat[adr].add(category)
                #dic_adr_to_site[adr].add(dummy_df.loc[adr][0])
    print("Total time to dict:", time.time()-starttime)

    # dic_user_to_cat = defaultdict(set)
    # dic_user_to_site = defaultdict(set)
    for i, user in enumerate(users):
        labels = set()
        for key in dic_adr_to_cat:
            if key in user.adr:
                cat = dic_adr_to_cat[key]
                #site = dic_adr_to_site[key]
                labels.add(list(cat)[0])
        if len(labels)==1:
            user_df.loc[i, 'category'] = list(cat)[0] #label[cat.pop()]
            # dic_user_to_cat[i].update(labels)
            # dic_user_to_site[i].update(site)
        elif len(labels)>1:
            user_df.loc[i, 'category'] = 'mixed'
            # dic_user_to_cat[i].update('mixed')
            # dic_user_to_site[i].update(" ")
